...
const HEAD_NAME = "Admin Charges";
const DEFAULT_BANK_NAME = "Bank of Maharashtra";

const ROWS = [
    ["14-Apr-25", "10922", 3834],
    ["15-May-25", "11434", 4134],
    ["14-Jun-25", "11964", 4127],
    ["14-Jul-25", "12535", 4134],
    ["13-Aug-25", "13110", 4134],
    ["15-Sep-25", "13659", 4127],
    ["14-Oct-25", "14296", 4116],
    ["14-Nov-25", "15139", 4134],
    ["15-Dec-25", "15718", 4070],
    ["14-Jan-26", "16265", 4134],
    ["14-Feb-26", "16836", 4080],
    ["13-Mar-26", "17506", 3798]
] as const;
...
async function main() {
    if (args.includes("--help")) {
        console.log(
            "Dry run: npm run create:admin-charges-journals\n" +
            "Apply:   npm run create:admin-charges-journals -- --apply\n" +
    ...
    const result = await prisma.$transaction(async tx => {
        ...
        if (!existingHead && !apply) {
            throw new Error(
                `${HEAD_NAME} JournalHead does not exist. Re-run with --apply to create it.`
            );
        }

        const adminLedger = existingHead
            ? apply
        ...
        const existingVouchers = await tx.voucher.findMany({
            where: { voucherNo: { in: ROWS.map(([, voucherNo]) => voucherNo) } },
        ...
        const preview = ROWS.map(([date, voucherNo, amount]) => ({
            date,
            voucherNo,
            amount,
        ...
        if (!apply) {
            return {
                branch,
                adminHead: adminHead.name,
                bankLedger,
                totalAdminChargesDebit: money(total),
                vouchers: preview,
                dryRun: true
            };
        }
        ...
        const created = [];
        for (const [date, voucherNo, amount] of ROWS) {
            const journalDate = parseDate(date);
            const importKey = `REPAIR_ADMIN_CHARGES_${voucherNo}`;
            ...
            const journal = await tx.journal.create({
                data: {
                    branchId: branch.id,
                    journalHeadId: adminHead.id,
                    importKey,
                    amount,
                    paymentMode: PaymentMode.ONLINE,
                    paymentThrough: PaymentType.CHEQUE,
                    remarks: `Admin Charges - voucher ${voucherNo}`,
                    journalDate,
            ...
            await LedgerService.syncCachedBalance(tx, adminLedger.id);
    ...
    if (result.dryRun) {
        console.log("Dry run only. Re-run with --apply to create all 12 vouchers.");
    }
}

...
export class LedgerService {
            ...
            const branchRows = await Promise.all(
                branches.map(async branch => {
                    ...
                    return {
                        ...
                        balanceType:
                            amountReceivable >= amountPayable
                                ? "RECEIVABLE"
                                : "PAYABLE",
                        createdAt:
                            branch.createdAt,
    ...
    static async getLedgerByAgencyId(
        ...
        switch (category) {
            ...
            case "CASH": {
                ...
                for (const ledger of agencyLedgers) {
        ...
        return {
            ...
            default: {
                try {
                                            ...
                                            ...statement.entries
                                                ...
                                                .map(row => {
                                                    ...
                                                    if (
                                                        ...
                                                        particular =
                                                            paymentMethod
                                                                ? `Payment received through ${String(paymentMethod)
                                                                    .toUpperCase()}`
                                                                : "Payment received";
                                                    }
                                                    ...
                                                    if (
                                                        ...
                                                        particular =
                                                            paymentMethod
                                                                ? `Payment made through ${String(paymentMethod)
                                                                    .toUpperCase()}`
                                                                : "Payment made";
                                                    }
                    ...
                    console.error("Error fetching accounting ledger for agency:", error);
                    throw error
                }
            }
        }
    ...
    static async getLedgerStatement(
        ...
        statementRows.push(...entries.map(entry => {
            ...
            return {
                ...
                paymentThrough:
                    transaction?.paymentThrough ?? null,
    ...
    private static async getPurchaseLedger(
        ...
        const label = labels[voucherType] || "Purchase";

        return this.getOrCreateLedger(client, {
            code: label === "Purchase"
                ? `PURCHASE-${branchCode}`
                : `${this.normalizeCode(label)}-${branchCode}`,
            name: `${label} - ${branchCode}`,
    ...
    static async postPurchaseApproval(
        ...
        const [
            ...
            this.getPurchaseLedger(tx, purchase.branchId, purchase.voucherType ?? VoucherType.PURCHASE),
    ...
    static async postDebitCreditNoteApproval(
        ...
        const notePurchaseTypes: VoucherType[] = [
        ...
        const noteLedger = isPurchase && note.purchaseVoucherType &&
            notePurchaseTypes.includes(note.purchaseVoucherType)
            ? await this.getPurchaseLedger(tx, note.branchId, note.purchaseVoucherType)

...
export const approveRecipe = async (req: Request, res: Response, next: NextFunction) => {
    try { return res.json({ success: true, message: "Recipe approved successfully", data: { recipe: await ManufacturingService.approveRecipe(actor(req), req.params.recipeId) } }); } catch (e) { next(e); }
};
export const rejectRecipe = async (req: Request, res: Response, next: NextFunction) => {
    try { return res.json({ success: true, message: "Recipe rejected successfully", data: { recipe: await ManufacturingService.rejectRecipe(actor(req), req.params.recipeId, req.body?.remarks) } }); } catch (e) { next(e); }
};
...
export const approveManufacture = async (req: Request, res: Response, next: NextFunction) => {
    try { return res.json({ success: true, message: "Manufacturing approved successfully", data: { manufacture: await ManufacturingService.approveManufacture(actor(req), req.params.manufactureId) } }); } catch (e) { next(e); }
};
export const rejectManufacture = async (req: Request, res: Response, next: NextFunction) => {
    try { return res.json({ success: true, message: "Manufacturing rejected successfully", data: { manufacture: await ManufacturingService.rejectManufacture(actor(req), req.params.manufactureId, req.body?.remarks) } }); } catch (e) { next(e); }
};
